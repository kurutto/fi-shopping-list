...
const Box = ({ variant, className, spBgWhite = false, ...props }: BoxProps) => {
  const baseStyle = cn(
    ...
    variant === "roundedMaxMd" &&
      " md:space-y-7 max-md:bg-white max-md:rounded-2xl max-md:px-4 max-md:py-6 max-md:space-y-5",
    variant === "roundedMaxMd" && (spBgWhite ? "shadow":"max-md:shadow-sp"),
    (variant === "rounded" || variant === "roundedMaxMd") && (spBgWhite ? "shadow":" md:shadow-pc max-md:shadow-sp"),
